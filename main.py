...
def run(process_id, args):
    with open(args.config_path) as f:
        reader = ruamel.yaml.YAML(typ='safe', pure=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--loss_conditional_iou', '-lci', type=float,
                        help='cost coef of conditional iou')
    parser.add_argument('--cost_conditional_iou', '-cci', type=float,
                        help='loss coef of conditional iou')
    ...
    parser.add_argument('--text_condition', '-tc', action='store_true', default=False,
                                  help='whether add text embedding to tgt(torch.zeros) variables entered in DETR.'
                                       'if True,  use averaged text embedding,'
                                       'if False, use zero tensors')
