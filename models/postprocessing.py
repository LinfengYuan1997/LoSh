...
class A2DSentencesPostProcess(nn.Module):
    ...
    def forward(self, outputs, resized_padded_sample_size, resized_sample_sizes, orig_sample_sizes):
        ...
        Parameters:
            outputs: raw outputs of the model (optional, long_short_outputs)
        ...
        pred_is_referred = long_outputs['pred_is_referred']
        prob = F.softmax(pred_is_referred, dim=-1)
        scores = prob[..., 0]

        long_pred_masks = long_outputs['pred_masks']
        long_pred_masks = F.interpolate(long_pred_masks, size=resized_padded_sample_size, mode="bilinear", align_corners=False)

        short_pred_masks = short_outputs['pred_masks']
        short_pred_masks = F.interpolate(short_pred_masks, size=resized_padded_sample_size, mode="bilinear", align_corners=False)


        long_pred_masks = (long_pred_masks.sigmoid() > 0.5)
        short_pred_masks = (short_pred_masks.sigmoid() > 0.5)
        pred_masks = long_pred_masks # * short_pred_masks

        # print("pred shape:", long_pred_masks.shape)


        # print("long:", long_pred_masks.sum())
        # print("short:", short_pred_masks.sum())
        # print("join:", pred_masks.sum())
